...
export const authLogout = () => {
    ...
    if (token === undefined){
        ...
        }, {headers: {'Authorization': `Token ${token}`}} ).catch(err => {console.error(err)});
...
export const authLogin = (username, password) => {
    return dispatch => {
      ...
      .then(res => {
        ...
        return axios.get(`${settings.API_SERVER}/api/auth/users/detail/`, {
          headers: { Authorization: `Token ${token}` }
        });
      })
      .then(userRes => {
        dispatch(storeUserDetails(userRes.data));  // Store user details
      })
      .catch(err => {
    ...
    return async (dispatch, getState) => {
      ...
      if (!token) {
        ...
        if (expirationDate <= new Date()) {
          ...
          if (!auth.user) {  
            try {
              const userRes = await axios.get(`${settings.API_SERVER}/api/auth/users/detail/`, {
                headers: { Authorization: `Token ${token}` },
              });
              dispatch(storeUserDetails(userRes.data));
            } catch (err) {
